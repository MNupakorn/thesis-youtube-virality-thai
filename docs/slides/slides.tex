...
\begin{frame}[plain]
  \begin{center}
    \includegraphics[width=2.0cm]{figures/kmitl_logo.png}\\[0.6em]
    {\large\bfseries\color{kmitlBlue}
      การศึกษาเชิงเปรียบเทียบของแบบจำลองทรานส์ฟอร์เมอร์ภาษาไทย\\
      สำหรับพยากรณ์ความไวรัลของวิดีโอ YouTube
    }\\[0.5em]
    {\normalsize\color{kmitlOrange}
      WangchanBERTa \ \textbullet\ \ PhayaThaiBERT \ \textbullet\ \ XLM-RoBERTa-large
    }\\[1.2em]
    {\footnotesize
      นายตรัยภูรินท์ สืบสุวรรณสาร \ (65050330) \\
      นายธีรนนท์ ไชยลังกา \ (65050417) \\
      นายภาณุเดช ภู่โทสนธิ์ \ (65050685)
    }\\[1.0em]
    {\scriptsize
      ภาควิชาสถิติประยุกต์และการวิเคราะห์ข้อมูล \\
      คณะวิทยาศาสตร์ \ \textbullet\ \ สถาบันเทคโนโลยีพระจอมเกล้าเจ้าคุณทหารลาดกระบัง \\[0.4em]
      อาจารย์ที่ปรึกษา: ผศ.\ ดร.\ พรพิมล ชัยวุฒิศักดิ์ \\[0.4em]
      ปีการศึกษา 2568
    }
  \end{center}
\end{frame}
...
\begin{frame}[shrink=8]{4.1 การเก็บรวบรวมข้อมูล (Data Collection)}
...
\begin{frame}{4.3 การแบ่งข้อมูล: Channel-grouped + Time-aware}
  ...
  \textbf{ป้องกัน leakage 2 ระดับ:} (1) channel level — ไม่ให้จำ ``ช่องนี้มักดัง''
  (2) time level — ทดสอบบนข้อมูล \emph{อนาคต}
\end{frame}
...
\begin{frame}[shrink=8]{5.2 Structural Features (27 ตัว)}
  \footnotesize
  \begin{columns}[T]
    \column{0.5\textwidth}
...
\begin{frame}[shrink=20]{5.3 Transformer Fine-tuning Procedure}
...
\begin{frame}[shrink=20]{5.4 Cloud GPU Strategy — Hugging Face Jobs}
...
\begin{frame}[shrink=5]{5.5 Imbalance Handling — \emph{train-only}}
  \begin{block}{หลักการ}
...
\begin{frame}[shrink=8]{5.6 Stacking Ensemble (15 base models)}
  \begin{columns}[T]
    \column{0.55\textwidth}
    \scriptsize
    \textbf{15 base models:}\\[0.2em]
    \begin{tabular}{@{}rl@{\quad}rl@{}}
      1.  & LR (struct)              & 9.  & XGB (struct+TF)\\
      2.  & LR (TF-IDF)              & 10. & Channel-prior\\
      3.  & LR (struct+TF)           & 11. & Hybrid MLP\\
      4.  & LGBM (struct)            & 12. & Hybrid GBM\\
      5.  & LGBM (TF-IDF)            & 13. & WangchanBERTa\\
      6.  & \hl{LGBM (struct+TF)} $\leftarrow$ best & 14. & \hl{PhayaThaiBERT} $\leftarrow$ best enc.\\
      7.  & XGB (struct)             & 15. & XLM-R-large\\
      8.  & XGB (TF-IDF)             &     & \\
    \end{tabular}
    \column{0.45\textwidth}
    \textbf{Meta-learner:}
...
\begin{frame}[shrink=5]{7.2 Leaderboard — Top 10 Models}
  \footnotesize
  \begin{tabular}{rlrr}
    \toprule
...
\begin{frame}[shrink=5]{7.3 ROC + PR Curves (Test Set)}
  \begin{center}
...
\begin{frame}[shrink=25]{7.6 Calibration Results}
  \begin{columns}[T]
    \column{0.55\textwidth}
...
\begin{frame}[shrink=30]{7.7 Reliability Diagram (Stacking)}
  \begin{center}
    \includegraphics[width=0.55\textwidth]{figures/reliability_stacking.png}
  \end{center}
  \begin{itemize}
...
\begin{frame}{8.2 Robustness Slices}
  \begin{center}
    \includegraphics[width=0.78\textwidth]{figures/robustness_slices.png}
  \end{center}
  \footnotesize
  \begin{itemize}\setlength\itemsep{0pt}
...
\begin{frame}[shrink=10]{8.4 5-fold GroupKFold Robustness}
...
\begin{frame}{9.1 SHAP Global Importance}
  ...
  \begin{columns}[T]
    \column{0.55\textwidth}
    \includegraphics[height=0.7\textheight,keepaspectratio]{figures/shap_summary.png}
    \column{0.45\textwidth}
    \textbf{ข้อสังเกต:}
    \begin{itemize}
...
\begin{frame}[shrink=5]{9.3 LIME — Local Explanation (ตัวอย่างจริง)}
  \footnotesize
...
\begin{frame}[shrink=8]{11.2 Implications สำหรับ Practice}
...
\begin{frame}[fragile]{[Backup] รายละเอียดของ Trainer Loop}
  \scriptsize
