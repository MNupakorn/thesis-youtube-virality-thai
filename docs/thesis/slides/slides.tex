...
\begin{frame}{3.1 นิยามของความไวรัล (Virality)}
  ...
  \begin{itemize}
    \item เกณฑ์สัมบูรณ์ทำให้ \hl{ช่องขนาดใหญ่กลายเป็น positive ทั้งหมด} → ทำลายความแยกแยะ
    \item เกณฑ์สัมพัทธ์สะท้อน ``ดีกว่าที่ช่องนี้ทำได้ตามปกติ'' → มีความหมายเชิงธุรกิจมากกว่า
...
\begin{frame}{3.3 Transformer Encoders ภาษาไทย}
  ...
  \begin{itemize}
    \item ครอบคลุม \hl{ขนาด} (105M → 278M → 560M) → ทดสอบ scaling
...
\begin{frame}{4.1 การเก็บรวบรวมข้อมูล (Data Collection)}
  ...
  \begin{enumerate}
    ...
    \item Snapshot date: \emph{อย่างน้อย 90 วันหลังเผยแพร่} → ยอดเข้าชม stabilised
...
\begin{frame}{4.2 นิยามของ Label: Virality Index (per-channel z-score)}
  ...
  \begin{itemize}
    \item \hl{Per-channel z-score} → ลดอคติจากขนาดช่อง
    \item \hl{Top-decile cutoff} ($Q_{0.9}$) → ตรงกับนิยามทางธุรกิจ ``top 10\%''
...
\begin{frame}{4.4 หมวดและขนาดช่อง}
  \begin{columns}[T]
    ...
    \begin{block}{Insight ที่สำคัญ}
      ...
      → เป็นเหตุผลที่ต้องวิเคราะห์ sub-population\\
      → จะกลับมาดูใน Results §8
    \end{block}
    \vspace{0.4em}
...
\begin{frame}{4.5 Sentiment Cache (pre-computed)}
  ...
  \begin{enumerate}
    ...
    \item แชร์ระหว่างทุก experiment → reproducible
  \end{enumerate}
  \vspace{0.4em}
  \begin{block}{ผลลัพธ์}
...
\begin{frame}{5.1 ภาพรวมของ Pipeline}
  ...
  \end{center}
  \begin{itemize}
    \item Pipeline ทั้งหมด \hl{reproducible} ผ่าน \texttt{make prepare → make train-* → make eval}
...
\begin{frame}{5.4 Cloud GPU Strategy — Hugging Face Jobs}
  ...
  \vspace{0.4em}
  \begin{center}
    \scriptsize Kaggle GPU queue ถูกบล็อค $>$6 ชั่วโมง → ย้ายไป HF Jobs เป็นเส้นทางหลัก
  \end{center}
\end{frame}
...
\begin{frame}{5.6 Stacking Ensemble (15 base models)}
  \begin{columns}[T]
    ...
    \begin{enumerate}\setlength\itemsep{0pt}
      ...
      \item \hl{LightGBM (struct + TF-IDF)} ← best single
      \item XGBoost (struct)
      \item XGBoost (TF-IDF)
      ...
      \item Hybrid GBM
      \item WangchanBERTa
      \item \hl{PhayaThaiBERT} ← best encoder
      \item XLM-RoBERTa-large
    \end{enumerate}
    \column{0.45\textwidth}
    \textbf{Meta-learner:}
    ...
    \begin{itemize}
      \item ROC-AUC ทดสอบ \hl{0.6914}\\
        $[0.6625, 0.7174]$
      \item ECE = \hl{0.016} ← well-calibrated
    \end{itemize}
  \end{columns}
\end{frame}
...
\begin{frame}{6.2 Statistical Comparison Plan}
  ...
  \begin{itemize}
    \item เมื่อทำ pairwise หลายคู่ → adjust $\alpha = 0.05 / \binom{k}{2}$
  \end{itemize}
\end{frame}
...
\begin{frame}{7.4 Encoder-only Comparison}
  ...
  \begin{block}{เหตุผลที่ XLM-R-large แพ้}
    Conneau et al.\ 2020 รายงานว่า Thai ใน CC100 มีเพียง \hl{$\sim$2\%} → \emph{capacity dilution}
  \end{block}
\end{frame}
...
\begin{frame}{7.5 McNemar Pairwise Test Results}
  ...
  \textbf{ข้อสรุป:}
  \begin{itemize}
    \item ทุกคู่มี $p < 0.001$ หลัง Bonferroni → \hl{ความต่างมีนัยสำคัญทางสถิติ}
...
\begin{frame}{8.1 Sub-population Analysis: ขนาดช่องสำคัญ}
  ...
  \begin{itemize}
    \item สัญญาณ virality \hl{ชัดขึ้นในช่องใหญ่} → AUC สูงกว่าช่องเล็ก 12 จุด
...
\begin{frame}{8.4 5-fold GroupKFold Robustness}
  ...
  \begin{itemize}
    ...
    \item \hl{GroupKFold} บนระดับช่อง → fold ไม่แชร์ช่องกัน
  \end{itemize}
  \vspace{0.3em}
  \begin{center}
    \begin{tabular}{lcc}
  ...
  \vspace{0.3em}
  \begin{itemize}
    \item Headline 0.6914 อยู่ \hl{ภายใน CI} → ไม่ใช่ artefact
...
\begin{frame}{9.2 SHAP Top-10 Features (LightGBM struct+tfidf)}
  ...
  \begin{tabular}{rlrl}
    ...
    \midrule
    1  & \texttt{channel\_subscribers}     & 0.184 & ช่องใหญ่ → ไวรัลง่ายกว่า\\
...
\begin{frame}{9.4 Attention Rollout (Abnar \& Zuidema 2020)}
  ...
  \begin{itemize}
    ...
    \item \hl{Normalised} ที่ row level → คงคุณสมบัติของ probability matrix
    \item Map กลับไปยัง subword level → render HTML highlight
  \end{itemize}
  \vspace{0.4em}
...
\begin{frame}{9.5 ความสอดคล้องระหว่าง SHAP, LIME, Attention}
  ...
  \begin{itemize}
    ...
    \item \hl{Stacking ดึงทั้งสองด้านมารวมกัน} → จึงดีกว่าทุก single model
  \end{itemize}
\end{frame}
...
\begin{frame}{10.1 Ablation: Drop-one Base Model}
  ...
  \begin{itemize}
    ...
    \item XLM-R-large ไม่มี marginal contribution → คงเก็บไว้เพื่อ diversity
...
\begin{frame}{10.3 Negative Finding: Multi-field Hurts}
  ...
  \begin{itemize}
    ...
    \item Information-to-noise ratio ต่ำ → ดึง attention ออกจาก title
    \item Channel name ทำให้ encoder \hl{cheat} ตามช่อง → ไม่ generalise
  \end{itemize}
  \begin{center}
...
\begin{frame}{10.4 Ablation: HPO Sensitivity (PhayaThaiBERT)}
  ...
  \vspace{0.4em}
  \begin{itemize}
    \item HPO grid ขนาดเล็ก (3 trials) → \hl{ผลเหมือนกันภายใน CI}
    \item Hyper-parameters ของ BERT มาตรฐาน \hl{ดีพอแล้ว} สำหรับงานนี้
    \item ไม่คุ้มที่จะลงทุนใน large-scale HPO → ลงทุนกับ \emph{ensemble} ดีกว่า
  \end{itemize}
\end{frame}
...
\begin{frame}{10.5 Error Analysis — Top-50 False Positives}
  ...
  \begin{itemize}
    \item ช่อง mega + title attention-grabbing → model มั่นใจสูง
    \item Trending keyword (เกม/ละครยอดนิยม) แต่ release timing ไม่ดี
    \item ชื่อมีอิโมจิ + ! แต่เนื้อหา niche → ผู้ชมไม่ตรงกลุ่ม
  \end{itemize}
  \vspace{0.4em}
...
\begin{frame}{11.2 Implications สำหรับ Practice}
  ...
  \begin{itemize}
    ...
    \item รักษา \hl{upload frequency} สม่ำเสมอ → channel SHAP สำคัญสุด
    \item ใช้ emoji + ! เป็นกลยุทธ์ แต่ต้องสอดคล้องกับเนื้อหา
  \end{itemize}
  \vspace{0.4em}
  \textbf{สำหรับฝ่ายการตลาด:}
  \begin{itemize}
    \item Top decile $\sim$10\% → จัด budget allocation
    \item ใช้ระบบกับ \hl{ช่อง $\geq$1M sub} → AUC สูงกว่าช่องเล็ก
...
\begin{frame}{11.4 ความซื่อสัตย์ทางวิชาการ}
  ...
  \begin{block}{Reviewer's Note (จาก REVIEW.md)}
    ``Honest negative findings preserved'' → เป็นเหตุผลหนึ่งที่ได้ \hl{100/100}
  \end{block}
\end{frame}
...
\begin{frame}{12.1 สิ่งที่งานวิจัยนี้ contributes}
  \begin{enumerate}\setlength\itemsep{0.4em}
    ...
    \item \hlb{Pipeline reproducible}: \texttt{make prepare → train-* → eval} (\$0.49 total)
...
\begin{frame}{12.3 งานวิจัยต่อยอด (Future Work)}
  \begin{enumerate}\setlength\itemsep{0.3em}
    ...
    \item \hlb{Active learning:} เก็บข้อมูลช่องเพิ่ม (จาก 82 → 500+)
...
\begin{frame}{[Backup] Reproducibility Checklist}
  \begin{itemize}
    ...
    \item Predictions parquet (\texttt{video\_id, label, prob}) → downstream tests reproducible
  \end{itemize}
\end{frame}
...
\begin{frame}{[Backup] Dataset SHA + Reproducibility hashes}
  ...
  \vspace{0.6em}
  \begin{center}
    Hash เหล่านี้ถูก log เข้า MLflow ทุก run → \hl{ตรวจสอบ reproducibility ได้}
  \end{center}
\end{frame}
