...
วิดีโอ, อัตราชั้นบวก 8.77\%) แบ่งเป็น 9 ส่วน ได้แก่ (1) สรุปประสิทธิภาพรวม
...
พร้อมช่วงความเชื่อมั่นบูตสแตรป 95\% และตัวชี้วัดประกอบที่ threshold ที่
optimal ของแต่ละแบบจำลอง
...
\begin{table}[H]
\centering
\caption{สรุปการกระจายของ ROC-AUC รายช่องสำหรับ \texttt{stacking\_lr\_calibrated}
...
threshold ที่ optimise F1$_{+}$ บน validation)
...
ระหว่างสองโหมดและให้ ROC-AUC สูงสุดที่ threshold ที่เลือกอย่างสมดุล
...
\small
\begin{tabularx}{\textwidth}{p{4cm}p{3cm}Xr}
\toprule
\textbf{Work} & \textbf{Platform / lang} & \textbf{Pre-publish?} & \textbf{Headline} \\
\midrule
Pinto et~al.\ \cite{pinto2013using}        & YouTube / EN  & ไม่ (ใช้ early views) & MRSE = 0.30 \\
Khosla et~al.\ \cite{khosla2014popularity} & Flickr / EN   & ใช่ (ภาพ) & Spearman $\rho$ = 0.81 \\
Trzci\'nski \& Rokita \cite{trzcinski2017popularity} & Facebook / EN & ใช่+early & $R^2$ = 0.65 (24h) \\
Hoiles et~al.\ \cite{hoiles2017engagement} & YouTube / EN  & ใช่ (meta) & RF reg. $R^2 \approx 0.5$ \\
\midrule
\textbf{This work}                          & YouTube / TH  & \textbf{ใช่} (text+meta only) & \textbf{ROC-AUC = 0.6914 [0.6625, 0.7174]} \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{itemize}[leftmargin=2em]
  \item งานก่อนหน้าส่วนใหญ่เป็นการ \emph{ถดถอย} (regression) ที่รายงาน $R^2$,
        ...
        \emph{จำแนกประเภท} ทวิภาค การเปรียบเทียบจึงทำได้เพียง
        \emph{เชิงโครงสร้าง}
...
\begin{itemize}[leftmargin=2em]
  \item คลังข้อมูลฝึกล่วงหน้าของ XLM-R-large กระจายอยู่ใน 100 ภาษา ทำให้สัดส่วน
        ภาษาไทยใน corpus ต่ำเพียง $\sim$2\% ตามรายงานของ Conneau et~al.\
        \cite{conneau2020xlmr} จึงเรียนรู้รูปแบบเฉพาะของภาษาไทยได้น้อยกว่า
...
ตามที่ Berger \& Milkman \cite{berger2012makes} แสดงในภาษาอังกฤษ สาเหตุที่
...
\begin{itemize}[leftmargin=2em]
  ...
  \item ฟีเจอร์ \texttt{log\_subscriber\_count} ติดอันดับ 3 ยืนยันการค้นพบของ
        Hoiles et~al.\ \cite{hoiles2017engagement} ในบริบทไทย
...
นี้ การประกอบหลายแบบจำลองจึงสำคัญ — แต่ละแบบจำลองให้ ``มุม'' ที่แตกต่างกัน
...
\midrule
$H_1$ & PhayaThaiBERT $>$ WangchanBERTa & \textbf{ยืนยัน} (0.6451 vs 0.6402, McNemar $p = 10^{-7}$) \\
$H_2$ & XLM-R-large $\geq$ monolingual Thai LMs & \textbf{ปฏิเสธ} (0.5450 ต่ำสุดในกลุ่ม) \\
$H_3$ & Stacking $>$ best single & \textbf{ยืนยัน} (0.6914 vs 0.6728, McNemar $p = 6.9\!\times\!10^{-53}$) \\
$H_4$ & รูปแบบความผิดพลาดของ encoders ต่างกัน & \textbf{ยืนยัน} (Cochran's Q $p = 9\!\times\!10^{-134}$) \\
\bottomrule
\end{tabularx}
\end{table}
...
ข้อสรุปนี้สอดคล้องกับงานของ Lin et~al.\ \cite{lin2022few} ที่พบรูปแบบเดียวกัน
