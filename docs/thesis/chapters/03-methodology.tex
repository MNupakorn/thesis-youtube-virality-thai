...
(6) แบบจำลองพื้นฐาน (7) การปรับจูนทรานส์ฟอร์เมอร์ภาษาไทย (8) การประกอบและ
...
DataReportal \cite{datareportal2025thailand}
...
Train (post-undersample)  & 6{,}964  & 25.00\% & ratio 3:1 negative:positive & ใช่ \\
...
ความตื่นเต้นของชื่อตามสมมติฐานของ Berger \& Milkman \cite{berger2012makes}
...
\texttt{paraphrase-multilingual-mpnet-base-v2} จาก SBERT \cite{reimers2020sbert}
...
\toprule
& \textbf{Structured} & \textbf{TF-IDF} & \textbf{Struct+TF-IDF} & \textbf{+Channel-prior} \\
\midrule
...
ค่าตั้งค่าอื่น ๆ ที่ใช้ร่วมกัน ได้แก่ \texttt{max\_length=128} (เพราะชื่อวิดีโอ
ภาษาไทยส่วนใหญ่สั้น), \texttt{epochs=3}, \texttt{warmup\_ratio=0.06},
\texttt{weight\_decay=0.01}, \texttt{eval\_strategy=epoch},
\texttt{load\_best\_model\_at\_end=True}, \texttt{metric\_for\_best\_model=eval\_roc\_auc},
...
ในชุดฝึกได้สมดุลคลาสไปแล้ว การทดลองเพิ่มเติมด้วย Focal Loss \cite{lin2017focal}
...
\section{การประกอบและการปรับเทียบ}
...
\begin{tikzpicture}[
  ...
  base/.style={rectangle,draw,rounded corners,fill=blue!8,minimum width=4cm,minimum height=0.55cm,font=\footnotesize},
  meta/.style={rectangle,draw,rounded corners,fill=red!15,minimum width=3cm,minimum height=0.7cm,font=\small},
  outbox/.style={rectangle,draw,rounded corners,fill=green!15,minimum width=3cm,minimum height=0.7cm,font=\small},
...
\node[meta,right=2.5cm of m8] (lr) {Meta-LR\\(C=1.0)};
...
ตัวชี้วัดทั้งหมดที่รายงานคำนวณจาก confusion matrix ที่ threshold $\theta$
ที่เลือกบน validation:
\begin{align}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{PR-AUC} (พื้นที่ใต้กราฟ Precision-Recall) สะท้อนการพยากรณ์
        ชั้นน้อยได้ดีกว่า ROC-AUC ในกรณีไม่สมดุลสูง
  \item \textbf{F1 ของชั้นบวก} (\texttt{f1\_pos}) ที่ threshold ที่ optimal
        บน validation
...
\subsection{การเลือก Threshold}

Threshold ของแต่ละแบบจำลองถูกเลือกบน \emph{validation} โดยสแกน
$\theta \in \{0.01, 0.02, \ldots, 0.99\}$ และเลือกค่าที่ maximise F1 ของ
ชั้นบวก จากนั้นใช้ threshold เดียวกันบน test เพื่อคำนวณ confusion matrix
...
แบ่งช่วง confidence เป็น 15 bins (\texttt{n\_bins=15}) ตามมาตรฐาน Naeini
et~al.\ \cite{naeini2015obtaining}
...
\begin{itemize}[leftmargin=2em]
  ...
  \item \textbf{Drop-one Stacking Ablation:} เอาแบบจำลองฐานออกทีละตัว
        แล้วเทรน meta-LR ใหม่ เพื่อวัดความสำคัญของแต่ละแบบจำลองในการประกอบ
\end{itemize}
...
\texttt{PYTHONHASHSEED=42}, \texttt{np.random.seed(42)}, \texttt{torch.manual\_seed(42)},
\texttt{transformers.set\_seed(42)}, \texttt{torch.backends.cudnn.deterministic=True}
